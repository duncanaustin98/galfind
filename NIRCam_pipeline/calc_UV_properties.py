...
def calc_UV_properties(
    ...
    fast=True,
    templates_arr=["fsps", "fsps_larson", "fsps_jades"],
    crop_key=None,
    overwrite=True,
):
    ...
    for pc_err in min_flux_pc_errs:
        ...
        for survey, xy_offset in zip(surveys, xy_offsets):
            cat = Catalogue.from_pipeline(
                ...
                n_loc_depth_samples=n_loc_depth_samples,
                templates_arr=templates_arr,
                fast=fast,
            )
            # calculate the extended source corrections
            cat.cat_path = cat.cat_path.replace(
                "eazy_fsps_larson", "EAZY"
            ).replace(
                ".fits", "_selection.fits"
            )  # ASSUMES THAT SELECTION HAS ALREADY BEEN PERFORMED!
...
if __name__ == "__main__":
    ...
    surveys = [
        "JADES-Deep-GS"
    ]  # ["CLIO", "SMACS-0723", "GLASS", "El-Gordo", "MACS-0416"]
