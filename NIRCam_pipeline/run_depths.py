...
def run_depths(surveys, version, xy_offsets, aper_diams, forced_phot_band, excl_bands, fast_depths, incl_offset = True):
    for survey, xy_offset in zip(surveys, xy_offsets):
        ...
        if incl_offset:
            data.calc_depths(xy_offset = xy_offset, aper_diams = aper_diams, fast_depths = fast_depths)
        else:
            data.calc_depths()
...
if __name__ == "__main__":
    ...
    run_depths(surveys, version, xy_offsets, aper_diams, forced_phot_band, excl_bands, fast_depths, incl_offset = True)
