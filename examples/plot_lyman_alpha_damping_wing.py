...
from galfind import DLA, wav_lyman_alpha
...
def plot_transmission(
    ...
    ax2.plot(
        wav_rest.to(
            u.km / u.s,
            equivalencies=u.doppler_relativistic(wav_lyman_alpha * u.AA),
        ),
        trans,
    )
    if annotate:
        ...
        ax2.set_xlabel(
            r"$\Delta v_{\mathrm{Ly\alpha}}~/~\mathrm{kms}^{-1}$"
        )  # (r"$\lambda_{\mathrm{obs}}~/~\mathrm{\AA}$")
...
def plot_lyman_alpha_damping_wing():
    z_arr = [8.0, 8.0, 8.0, 8.0]
    x_HI_arr = [0.5, 0.5, 0.5, 0.5]
    helium_mass_frac_arr = [0.25, 0.25, 0.25, 0.25]
    R_b_arr = [0.0, 0.0, 0.0, 0.0] * u.Mpc
    wav_rest = np.linspace(1_200.0, 1_268.0, 100) * u.AA
    velocity_offset_arr = [0.0, 200.0, 400.0, 800.0] * u.km / u.s
    legend_kwargs = {"bbox_to_anchor": (0.5, -0.2), "loc": "upper center"}

    fig, ax = plt.subplots()
    ax2 = ax.twiny()
    for i, (z, R_b, x_HI, helium_mass_frac, velocity_offset) in enumerate(
        zip(
            z_arr, R_b_arr, x_HI_arr, helium_mass_frac_arr, velocity_offset_arr
        )
    ):
        trans = [
            get_transmission(
                tau_DW(
                    velocity_offset.to(
                        u.AA, equivalencies=u.doppler_relativistic(wav)
                    ),
                    z,
                    R_b,
                    x_HI,
        ...
        plot_transmission(
            ...
            trans,
            z,
            label=f"z={z}, R_b={R_b}, x_HI={x_HI}, Y={helium_mass_frac}, Δv={velocity_offset}",
            show=True if i == len(z_arr) - 1 else False,
...
def plot_proximate_DLA():
    ...
    N_HI_arr = [10**21.5, 10**23.0] / (u.cm**2)
    x_HI_arr = [1.0, 1.0]
    R_b_arr = [0.0, 0.0, 0.0, 0.0] * u.Mpc
    helium_mass_frac_arr = [0.25, 0.25, 0.25, 0.0]
    velocity_offset_arr = [0.0, 0.0] * u.km / u.s
    wav_rest = np.linspace(1_216.0, 1_400.0, 1000) * u.AA
    legend_kwargs = {"bbox_to_anchor": (0.5, -0.2), "loc": "upper center"}
    ...
    for i, (
        ...
        plot_transmission(
            ...
            trans,
            z,
            label=f"DLA: T={gas_temp}, N_HI={N_HI}, Δv={velocity_offset}",
            show=True if i == len(z_arr) - 1 else False,
            legend_kwargs=legend_kwargs,
        )
...
def plot_voigt_profile():
    ...
    b = 100 * u.km / u.s
    wav_rest = np.linspace(1_213.0, 1_218.0, 1000) * u.AA
    # delta_lambda = delta_lambda_lyman_alpha_from_gas_temp(gas_temp)
