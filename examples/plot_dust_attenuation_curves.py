...
def plot_color_color_dust_arrow(ax, z, colour_x_name, colour_y_name, arrow_pos, E_BminusV, dust_law = Calzetti00(), beta = -2.2, M_UV = -22., arrow_kwargs = {"c": "black"}):
    ...
    arrow = Arrow(arrow_pos[0], arrow_pos[1], dx = colour_x - colour_x_0, dy = colour_y - colour_y_0, width = 0.15, color = arrow_kwargs["c"])
    ax.add_patch(arrow)
    ax.annotate(f"E(B-V)={E_BminusV:.2f}", (1., 1.), xycoords = arrow, ha = 'center', va = 'top', **arrow_kwargs)

if __name__ == "__main__":
    redshifts = [6.5, 8.5, 10.5, 12.5]
    fig, ax = plt.subplots()
    for z in redshifts:
        plot_color_color_dust_arrow(ax, z, "f150W-f277W", "f277W-f444W", [0.5, 0.5], 0.5)