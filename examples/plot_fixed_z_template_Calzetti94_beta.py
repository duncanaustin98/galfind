...
def calc_mock_beta_phot(
    z_arr,
    template_set,
    incl_bands,
    rest_UV_wav_lims_arr,
    m_UV_norm,
    fixed_depth_mag,
    std_NIRCam_bands=[
        "f090W",
    ...
    NIRCam_excl_bands = [
        band for band in NIRCam_excl_bands if band not in incl_bands
    ]
    WFC3_IR_excl_bands = [
        "f098M",
        "f105W",
    ...
    WFC3_IR_excl_bands = [
        band for band in WFC3_IR_excl_bands if band not in incl_bands
    ]
    instrument = NIRCam(excl_bands=NIRCam_excl_bands) + WFC3_IR(
        excl_bands=WFC3_IR_excl_bands
    )
    if incl_bands == []:
        title = "8_NIRCam"
    else:
    ...
    if not Path(f"{output_dir}/{title}.ecsv").is_file():
        mock_sed_rest_set = Mock_SED_rest_template_set.load_EAZY_in_template(
        ...
        with tqdm(
            ...
            * len(z_arr),
            desc=f"Calculating β for bands = {title}",
            leave=False,
        ) as pbar:
            for i, rest_UV_wav_lims in enumerate(rest_UV_wav_lims_arr):
                ...
                for j, mock_sed_rest in enumerate(mock_sed_rest_set):
                    for k, z in enumerate(z_arr):
                        ...
                        phot_rest = Photometry_rest.from_phot(
                            mock_sed_rest.mock_photometry,
                            z,
                            rest_UV_wav_lims=rest_UV_wav_lims,
                        )
                        beta_phot_loc[j * len(z_arr) + k] = (
                            phot_rest.basic_beta_calc(incl_errs=incl_errs)
                        )
                        pbar.update(1)
                ...
                **{
                    "template_name": np.array(
                        [
                            ...
                            for mock_sed_rest in mock_sed_rest_set
                            for i in range(len(z_arr))
                        ]
                    ),
                    "z": np.tile(z_arr, len(mock_sed_rest_set)),
                },
                **beta_phot,
            }
        )
...
def calc_C94_beta(template_set, m_UV_norm):
    ...
    os.makedirs(output_dir, exist_ok=True)
    out_tab.write(
        f"{output_dir}/{template_set}_Calzetti+94.ecsv", overwrite=True
    )
...
def plot_dbeta(
    ...
    cmap_name="Oranges",
    plot_indices=[],
    output_dir=f"{config['DEFAULT']['GALFIND_WORK']}/Beta_paper/Delta_beta_fsps_larson",
):
    mock_sed_rest_set = Mock_SED_rest_template_set.load_EAZY_in_templates(
        m_UV_norm, template_set
    )
    ...
    for i, plot_index in enumerate(plot_indices):
        ...
        loc_phot_tab = phot_tab[
            phot_tab["template_name"] == mock_sed_rest.template_name
        ]
        C94_beta = spec_tab[
            spec_tab["template_name"] == mock_sed_rest.template_name
        ]["Beta_C94"]
        ax.plot(
            loc_phot_tab["z"],
            np.array(
                loc_phot_tab[
                    f"Beta_phot_{'-'.join([str(int(wav)) for wav in rest_UV_wav_lims.value])}Angstrom"
                ]
            )
            - C94_beta,
            label=mock_sed_rest.template_name,
            c=cmap[int(i)],
        )
    if incl_bands == []:
        title = "8_NIRCam"
    ...
    if annotate:
        ...
        ax.set_ylabel(
            rf"Δβ ({'-'.join([str(int(wav)) for wav in rest_UV_wav_lims.value])}$\AA$ $-$ C+94)"
        )
...
if __name__ == "__main__":
    ...
    z_arr = np.linspace(6.5, 12.0, 100)
    min_pc_err = 10
    template_set = "fsps_larson"
    rest_UV_wav_lims_arr = [[1250, 3000], [1268, 2580]] * u.Angstrom
    m_UV_norm = 25.0  # arbitrary if incl_errs == False
    fixed_depth_mag = 30.0
    incl_errs = False
    ...
    for incl_bands in tqdm(
        band_combinations,
        total=len(band_combinations),
        desc=f"Calculating β for {template_set} λ_rest={str(rest_UV_wav_lims_arr)} incl_errs={incl_errs}",
        leave=False,
    ):
        calc_mock_beta_phot(
            z_arr,
        ...
        plot_dbeta(
            ...
            template_set,
            incl_bands,
            rest_UV_wav_lims_arr[0],
            m_UV_norm,
            fixed_depth_mag,
            incl_errs,
            cmap_name="Blues_r",
            plot_indices=np.linspace(12, 17, 6),
        )
