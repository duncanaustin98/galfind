...
from galfind import Instrument, NIRCam, ACS_WFC, Photometry, Photometry_rest, config, useful_funcs_austind
...
def main(cat_path, sample_key, instrument_name, templates = "fsps_larson", save = True):
    # load catalogue
    ...
    for true_z, photo_z, row in tqdm(zip(true_redshifts, obs_redshifts, tab), total = len(true_redshifts), desc = f"Calculating beta/M_UV for {survey} {templates} JAGUAR catalogue"):
        ...
        raw_m_UV = raw_phot_rest_obj.basic_m_UV_calc(1., incl_errs = False)
        raw_M_UV = raw_phot_rest_obj.basic_M_UV_calc(1., incl_errs = False)
        ...
        # z fixed + scattered
        scattered_phot_rest_obj_zfix = Photometry_rest.from_phot(scattered_phot_obj, float(true_z))
        scattered_beta_zfix = scattered_phot_rest_obj_zfix.basic_beta_calc(incl_errs = True)
        scattered_m_UV_zfix = scattered_phot_rest_obj_zfix.basic_m_UV_calc(1., incl_errs = True)
        scattered_M_UV_zfix = scattered_phot_rest_obj_zfix.basic_M_UV_calc(1., incl_errs = True)
        ...
        scattered_phot_rest_obj = Photometry_rest.from_phot(scattered_phot_obj, float(photo_z))
        scattered_beta = scattered_phot_rest_obj.basic_beta_calc(incl_errs = True)
        scattered_m_UV = scattered_phot_rest_obj.basic_m_UV_calc(1., incl_errs = True)
        scattered_M_UV = scattered_phot_rest_obj.basic_M_UV_calc(1., incl_errs = True)
    ...
    if save:
        ...
        UV_tab = Table([IDs, beta_raw_arr, m_UV_raw_arr, M_UV_raw_arr, beta_scattered_arr, m_UV_scattered_arr, M_UV_scattered_arr, beta_scattered_arr_zfix, m_UV_scattered_arr_zfix, M_UV_scattered_arr_zfix], \
                    names=('ID', 'Beta_true', 'm_UV_true', 'M_UV_true', f'Beta_EAZY_{templates}', f'm_UV_EAZY_{templates}', f'M_UV_EAZY_{templates}', f'Beta_EAZY_{templates}_zfix', f'm_UV_EAZY_{templates}_zfix', f'M_UV_EAZY_{templates}_zfix'), \
...
if __name__ == "__main__":
    ...
    surveys = ["CEERS", "CLIO", "El-Gordo", "GLASS", "MACS-0416", "NEP", "NGDEEP", "SMACS-0723", "JADES"]
