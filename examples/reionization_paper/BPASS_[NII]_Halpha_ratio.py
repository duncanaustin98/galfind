...
def main(imf = "imf135_300", log_ages = [7.], m_UV_norm = 26. * u.ABmag, logU_arr = [-4., -3.5, -3., -2.5, -2., -1.5, -1.], \
        metallicity_arr = ["zem4", "z001", "z010"]): # ["zem5", "zem4", "z001", "z002", "z003", "z004", "z006", "z008", "z010", "z020", "z030", "z040"]
    from galfind import Mock_SED_rest_template_set, Mock_SED_obs, line_diagnostics
    NII_Halpha = {}
    OIII_Hbeta = {}
    for metallicity in metallicity_arr:
        ...
        for logU in logU_arr:
            BPASS_template_set_rest = Mock_SED_rest_template_set.load_bpass_in_templates(metallicity = metallicity, \
                logU = logU, imf = imf, log_ages = log_ages, m_UV_norm = m_UV_norm, bpass_version = "2.2_cloudy")
            print(f"{logU=}")
            for template, log_age in zip(BPASS_template_set_rest.SED_arr, log_ages):
                print(f"{log_age=}")
            ...
            NII_Halpha[metallicity].append((NII_EW / Halpha_EW).to(u.dimensionless_unscaled).value)
        # get OIII_Hbeta
        OIII_Hbeta[metallicity] = OIII_Hbeta_from_EW_tab(logU_arr, log_ages[-1], metallicity)
...
def plot_BPT_AGN_OIII_Hbeta_NII_Halpha(ax, author_year = "Kauffmann2013", kwargs = {"c": "black"}):
    x_arr = np.linspace(-4., 0., 1_000)
...
def plot_BPT(logU_arr, NII_Halpha_dict, OIII_Hbeta_dict, age):
    fig, ax = plt.subplots()
    for (metallicity, NII_Halpha), (metallicity_2, OIII_Hbeta) in zip(NII_Halpha_dict.items(), OIII_Hbeta_dict.items()):
...
def BPASS_BPT_v2_1(metallicities = "All", log_nH = 2., log_age = 6.5, \
    ...
    if metallicities == "All":
        ...
        tab_paths = [f"{bpass_dir}/Optical_data_bin_{metallicity}.dat" for metallicity in metallicities]
    for tab_path in tab_paths:
        metallicity = tab_path.split("_")[-1].replace(".dat", "")
        tab = Table.read(tab_path, format = "ascii.commented_header")
        selection_mask = ((tab["#3)log(n_H/cm^-3)"] == log_nH) & (tab["#4)log(Age/yr)"] == log_age))
        ...
        ax.plot(np.log10(NII_Halpha), np.log10(OIII_Hbeta), label = f"Z={metallicity}")
    
    plot_BPT_AGN_OIII_Hbeta_NII_Halpha(ax)
    ax.legend()
    ax.set_title(f"Age = {((10 ** log_age) * u.yr).to(u.Myr).value} Myr, nH = {10 ** log_nH} / cm3")
    ax.set_xlabel(r"$\log($[NII]$_{\lambda 6583}$/H$\alpha)$")
    ax.set_ylabel(r"$\log($[OIII]$_{\lambda 5007}$/H$\beta)$")
    ax.set_xlim(-2.5, 0.)
    ax.set_ylim(-2., 1.5)
...
def BPASS_NII_Halpha_v2_1(metallicities = "All", log_nH = 2., log_age = 6.5, \
        bpass_dir = "/raid/scratch/data/BPASS/BPASSv2.1_nebula_release-02-18-Kiwi/Optical/bin"):
    fig, ax = plt.subplots()
    if metallicities == "All":
        tab_paths = glob.glob(f"{bpass_dir}/Optical_data_bin_*.dat")
    ...
    for tab_path in tab_paths:
        ...
        tab.sort("#2)log(U)")
        logU = tab["#2)log(U)"]
        NII_Halpha = (tab["#7)NII[6584A]"] + tab["#5)NII[6548A]"]) / tab["#19)Ha[6563A]"]
