...
def Halpha_template_phot_color_diff(
    ...
    cat = Catalogue.from_pipeline(
        ...
        loc_depth_min_flux_pc_errs=[min_pc_err],
        crop_by=crop_by,
        timed=timed,
    ...
    colours_arr = [
        ...
        .SED.calc_colour(filters_colour)
        .value
        for gal in tqdm(
            deepcopy(cat), desc="Calculating template colours", total=len(cat)
        )
    ]
    colour_tab = Table(
        {"IDs": cat.ID, "-".join(colour_arr): colours_arr}, dtype=[int, float]
    )
    colour_tab.write(
        f"{survey}_{version}_{'+'.join(instruments)}_{'-'.join(colour_arr)}.fits"
    )
...
def main():
    ...
    cat_type = "loc_depth"
    survey = (
        "JOF"  # ["JADES-Deep-GS+JEMS+SMILES"] #[config["DEFAULT"]["SURVEY"]]
    )
    ...
    Halpha_template_phot_color_diff(
        survey,
        version,
        instruments,
        aper_diams,
