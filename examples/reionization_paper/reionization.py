...
def M_UV_to_L_UV(M_UV):  # redshift independent
    return (
        (
            (400.0 * np.pi * 10 ** ((M_UV - u.Jy.to(u.ABmag)) / -2.5))
            * u.Jy
            * u.pc**2
        )
        .to(u.erg / (u.s * u.Hz))
...
def flat_xi_ion(z, M_UV, log_xi_ion=25.2):  # u.Hz / u.erg
    return np.full(len(M_UV), 10.0**log_xi_ion)
...
def schechter_function(M_UV, M_star, phi_star, alpha):
    return (
        (phi_star * np.log(10) / 2.5)
        * ((10.0 ** (0.4 * (M_star - M_UV))) ** (alpha + 1.0))
        * np.exp(-(10.0 ** (0.4 * (M_star - M_UV))))
    )


def Bouwens15_UVLF_evolution(z, M_UV):
    # calculate UVLF params
    M_star = -20.95 + 0.01 * (z - 6.0)
    phi_star = 0.47 * 10 ** (-0.27 * (z - 6.0) - 3.0)  # * u.Mpc ** -3
    alpha = -1.87 - 0.1 * (z - 6.0)
    return schechter_function(M_UV, M_star, phi_star, alpha)


def plot_UVLF(
    ax, z, M_UV, parametrization="Bouwens+15", legend=True, save=True
):
...
def const_clumping_factor(z, clumping_factor=3.0):
...
def calc_n_dot_ion(
    ...
    M_UV_lim,
    fesc_func=flat_fesc,
    xi_ion_func=flat_xi_ion,
    UVLF_func=Bouwens15_UVLF_evolution,
):
    M_UV = np.linspace(-30.0, M_UV_lim, 1_000)
    integrand = (
        fesc_func(z, M_UV)
        * xi_ion_func(z, M_UV)
        * UVLF_func(z, M_UV)
...
def n_dot_ion_stable_reionization(
    z,
    Hydrogen_abundance,
    astropy_cosmo,
    Q_HII,
    electron_temp=1e4 * u.K,
    recomb_coeff_func=case_B_recomb_coeff,
    clumping_factor_func=Shull2012_clumping_factor,
):
    return (
        (
            calc_n_H(z, Hydrogen_abundance, astropy_cosmo)
            * Q_HII
            / calc_trec(
                z,
                Hydrogen_abundance,
...
def plot_n_dot_ion_vs_M_UV_lim(
    ax,
    z_arr,
    M_UV_lim_arr,
    Hydrogen_abundance,
    ...
    for z in z_arr:
        n_dot_ion_arr = [
            calc_n_dot_ion(
                z, M_UV_lim, fesc_func, xi_ion_func, UVLF_func
            ).value
            for M_UV_lim in M_UV_lim_arr
        ]
        plot = ax.plot(M_UV_lim_arr, np.log10(n_dot_ion_arr), label=f"z = {z}")
        for Q_HII in stable_Q_HII_lines:
            n_dot_stable_ion = n_dot_ion_stable_reionization(
                z,
                Hydrogen_abundance,
                astropy_cosmo,
                Q_HII,
    ...
    ax.set_ylabel(
        r"$\log_{10}(\dot{n}_{\mathrm{ion}}~/~s^{-1}~\mathrm{Mpc}^{-3})$"
    )
    if legend:
        ax.legend()
    if save:
...
def calc_n_H(z, Hydrogen_abundance, astropy_cosmo):
    return (
        astropy_cosmo.Ob(z)
        * Hydrogen_abundance
        * astropy_cosmo.critical_density(z)
        / const.m_p
    ).to(u.cm**-3)
...
def calc_trec(
    ...
    return (
        1.0
        / (
            (1.0 + (1.0 - Hydrogen_abundance) / (4.0 * Hydrogen_abundance))
            * calc_n_H(z, Hydrogen_abundance, astropy_cosmo)
            * recomb_coeff_func(electron_temp)
            * clumping_factor_func(z)
        )
    ).to(u.s)


def calc_stable_Q_HII_z(
    Q_HII,
    M_UV_lim,
    Hydrogen_abundance,
    astropy_cosmo,
    ...
    z_arr = np.linspace(0.0, z_start, 1_000)
    stable_Q_HII_list = [
        (
            calc_n_dot_ion(z, M_UV_lim, fesc_func, xi_ion_func, UVLF_func)
            * calc_trec(
                z,
                Hydrogen_abundance,
                astropy_cosmo,
    ...
    return z_arr[
        int(
            min(
                ((i, value) for i, value in enumerate(stable_Q_HII_list)),
                key=lambda x: abs(x[1] - Q_HII),
            )[0]
        )
    ]
...
def main():
    # plot UV LF
    z_arr = [7.0, 8.0, 9.0, 10.0]
    M_UV = np.linspace(-21.0, -15.0, 100)
    ...
    print(calc_stable_Q_HII_z(1.0, -17.0, Hydrogen_abundance, astropy_cosmo))
    ...
    print(
        calc_trec(
            ...
            Hydrogen_abundance,
            astropy_cosmo,
            clumping_factor_func=const_clumping_factor,
        ).to(u.Myr)
    )
