...
def load_DJA_spectra(ra_range = None, dec_range = None):
   ...
   #spec = 
    ...
    [spec[0].plot_spectrum() for spec, SNR in zip(spec_cat, cat_UV_SNR) if SNR > 50.]
    return [spec[0] for spec, UV_SNR in zip(spec_cat, cat_UV_SNR) if UV_SNR > 50.]#, '1210_13197', '4557_-184']]
...
def main():
    ...
    for spec in spectra:
        ...
        mock_phot_JOF = sed_obs.create_mock_phot(NIRCam_JOF, [30. for i in range(len(NIRCam_JOF))])
        phot_rest_JOF = Photometry_rest.from_phot(mock_phot_JOF, spec.z)
