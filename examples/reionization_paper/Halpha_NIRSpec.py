...
def main():
    ...
    for spec in spectra:
        ...
        _beta_JOF = phot_rest_JOF.calc_beta_phot([1_250., 3_000.] * u.AA, single_iter = True)[1][1]
        _Halpha_JOF, Halpha_JOF_kwargs = phot_rest_JOF.calc_EW_rest_optical(["Halpha"], frame = "rest", single_iter = True)
        #breakpoint()
    ...
    tab = Table({"ID": [spec.src_name for spec in spectra], "z_PRISM": [spec.z for spec in spectra], "beta_mock_phot": beta_JOF, "Halpha_NII_mock_phot": Halpha_JOF, "SNR_UV": [spec.SNR for spec in spectra]})
    print(len(tab))
    #breakpoint()
    [spec.plot_spectrum() for spec in spectra]
    tab["Halpha_NII_PRISM_DJA"] = [spec.fit_data["eqwidth"]["line Ha+NII"][0] / (1. + spec.z) for spec in spectra]
