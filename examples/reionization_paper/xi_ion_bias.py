    ...
    for i, sed_rest in enumerate([sed_rest_template_set[0]]):
        ...
        if plot:
            ...
            for wav in list(line_diagnostics["Halpha"]["feature_wavs"].to(u.AA).value) \
                ...
                ax.axvline(wav, 0., 1.)
            ax.set_ylim(26.5, 20.)
            plt.savefig(f"test_plots/SED_rest_test_{sed_rest.template_name}.png")
            plt.clf()
        ...
        Halpha_EWrest_arr_obs = [deepcopy(phot_rest).calc_EW_rest_optical(["Halpha"], frame = "rest", single_iter = True)[0] \
...
def plot_band_z_coverage(phot_rest_arr):
    ...
    kwargs_arr = [deepcopy(phot_rest).calc_EW_rest_optical(["Halpha"], single_iter = True)[1] for phot_rest in phot_rest_arr]
    ...
    for i, band in enumerate(phot_rest_arr[0].instrument):
        ...
        #colour = cmap[i]
        _cont_band_z_coverage = np.array([z for i, z in enumerate(z_arr) if band_name in cont_bands_arr[i]])
        if len(_cont_band_z_coverage) > 1:
            z_coverage = calc_z_coverage(_cont_band_z_coverage, z_arr, band_name)
            plot_band_coverage(ax, z_coverage, band, coverage_type = "continuum", label = True if "continuum" not in already_labelled else False)
            ...
            plotted_cont = True
        _em_band_z_coverage = np.array([z for i, z in enumerate(z_arr) if band_name in em_bands_arr[i]])
        if len(_em_band_z_coverage) > 1:
            z_coverage = calc_z_coverage(_em_band_z_coverage, z_arr, band_name)
            plot_band_coverage(ax, z_coverage, band, coverage_type = "emission", label = True if "emission" not in already_labelled else False)
    ...
    for line in strong_optical_lines:
        ax.axhline(line_diagnostics[line]["line_wav"].to(u.AA).value, c = "darkgreen", lw = 2., ls = "--", zorder = 120.)
        if line in ["Halpha", "[OIII]-4959"]:
            line_name = {"Halpha": r"H$\alpha$", "[OIII]-4959": r"[OIII]+H$\beta$"}
            ax.text(ax.get_xlim()[1], line_diagnostics[line]["line_wav"].to(u.AA).value, line_name[line], size = 12., ha = "right", va = "center", zorder = 150., \
...
def calc_z_coverage(z, orig_z_arr, band_name = None):
    ...
    start_indices = [i for i in matching_indices if i - 1 not in matching_indices and i + 1 in matching_indices]
    end_indices = [i for i in matching_indices if i + 1 not in matching_indices and i - 1 in matching_indices]
    assert len(start_indices) == len(end_indices)
    #split_arr = np.array_split(matching_indices, end_indices)
    z_coverage = [(orig_z_arr[start_index], orig_z_arr[end_index]) \
        for start_index, end_index in zip(start_indices, end_indices)]
    return z_coverage

def plot_band_coverage(ax, z_coverage, band, delta_z = 0.01, coverage_type = "emission", label = False):
    if coverage_type == "emission":
        kwargs = {"color": "red", "ec": "black", "lw": 2.5, "hatch": "X", "alpha": 0.5, "zorder": 100.}
        label_colour = "pink"
    ...
    for z_coverage_ in z_coverage:
        ...
        ax.fill_between(z_range, band.WavelengthLower50.to(u.AA).value / (1. + z_range), \
            band.WavelengthUpper50.to(u.AA).value / (1. + z_range), **kwargs)
        z_mean = np.mean(z_range)
        ax.text(z_mean, ((band.WavelengthLower50.to(u.AA).value + band.WavelengthUpper50.to(u.AA).value) / 2.) / (1. + z_mean), \
            band.band_name, c = label_colour, size = 10., va = "center", ha = "center", weight = "bold", zorder = 150., path_effects = [pe.withStroke(linewidth=3, foreground="black")])

def main():
    z_arr = np.linspace(4.5, 7., 5001)
