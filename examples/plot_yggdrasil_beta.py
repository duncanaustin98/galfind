...
def plot_yggdrasil_age_vs_beta(grid_dir, imfs, fcovs, sfhs, solar_Z = 0.02):
    ...
    if not Path(in_path).is_file():
        out_tab = Table({"IMF": [], "fesc": [], "sfh": [], "age": [], "beta": []})
        for i, imf in enumerate(imfs):
            ...
            for j, fcov in enumerate(fcovs):
                ...
                for k, sfh in enumerate(sfhs):
                    ...
                    label = label.replace(f"_SFR_{sfh}_Spectra", "").replace("_kroupa_IMF", "").split("_")[0]
                    x = np.log10((ages * u.Myr).to(u.yr).value)
                    y = np.array(beta)
                    tab = Table({"IMF": np.full(len(x), imf), "fesc": np.full(len(x), 1 - float(fcov)), "sfh": np.full(len(x), sfh), "age": x, "beta": y})
                    if len(out_tab) == 0:
                        out_tab = tab
                        print("here")
                    else:
    ...
    for i, imf in enumerate(imfs):
        ...
        for j, fcov in enumerate(fcovs):
            if not ("PopIII" in imf and fcov == "1"):
                ...
                for sfh in sfhs:
                    #print(out_tab)
                    tab_ = out_tab[((out_tab["IMF"] == imf) & (out_tab["fesc"] == 1 - float(fcov)) & (out_tab["sfh"] == sfh))]
                    x = tab_["age"]
                    y = tab_["beta"]
                    #print(x, y)
                    if j == 0:
                        ...
                        if len(label.split("=")) > 1:
                            ...
                            label_split[0] = r"$Z$"
                            label_split[1] = f"{float(label_split[1]) / solar_Z : .2f}" + r"Z$_{\odot}$"
                            print(label_split)
    ...
    age_lims = [6., 9.]
    alpha_base = 0.8
    for i, (beta, beta_l1, beta_u1, colour, label) in enumerate(zip(beta_arr, beta_l1_arr, beta_u1_arr, colours, labels)):
        ax.fill_between(age_lims, [beta - beta_l1] * 2, [beta + beta_u1] * 2, color = colour, alpha = alpha_base) # β(M_UV=-19)
        ax.text(age_lims[-1] - 0.5, beta, label, c = "white", path_effects = [pe.withStroke(linewidth = 3., foreground = "black")], ha = "center", va = "center")
    ...
    for line, fcov in zip(ls, fcovs):
        ax_line, = plt.plot(-99., -99., ls = line, lw = 2., c = "black")
        lines.append(ax_line)
        labels.append(r"$f_{\mathrm{esc, LyC}}=%.1f$" % (1. - float(fcov)))
    ls_legend = plt.legend(lines, labels, loc = "lower right", ncol = len(fcovs), frameon = False)
    imf_legend = plt.legend(ncol = 3, bbox_to_anchor = (1.0, 1.0), loc = "upper right", framealpha = 1., frameon = False)
...
if __name__ == "__main__":
    ...
    imfs = ["Z=0.0004_kroupa_IMF", "Z=0.004_kroupa_IMF", "Z=0.02_kroupa_IMF", "PopIII.1", "PopIII.2", "PopIII_kroupa_IMF"] #, "Z=0.008_kroupa_IMF", 
    fcovs = ["0", "0.5", "1"]
