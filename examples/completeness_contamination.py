...
bin_dict = {
    "int": {
        "mass": np.linspace(6.0, 10.5, 10),
        "beta": np.linspace(-3.5, -0.5, 10),
        "M_UV": np.linspace(-22.5, -16.0, 10),
    },
    "obs": {
        "mass": np.linspace(6.5, 10.5, 8),
        "beta": np.linspace(-7.0, 0.0, 8),
        "M_UV": np.linspace(-22.5, -16.5, 8),
    },
}
frame_names = {
    "int": {
    ...
    "obs": {
        "z": "zbest_fsps_larson",
        "mass": "stellar_mass_50",
        "beta": "Beta_EAZY_fsps_larson",
        "M_UV": "M_UV_EAZY_fsps_larson",
    },
}
var_labels = {
    "z": "Redshift, z",
    "mass": r"$\log_{10}(M_{\star}/\mathrm{M}_{\odot})$",
    "beta": r"$\beta$",
    "M_UV": r"$M_{\mathrm{UV}}$",
}
mass_corr_dict = {
    "lognorm_zfix": {
        "high-z": {"mu": 0.2436, "sigma": 0.0},
        "interlopers": {"mu": 0.5271, "sigma": 0.0},
    }
}
...
def MD14_mass_conversion(in_IMF, out_IMF):
    if in_IMF == "Salpeter":
        conv_factor = {"Salpeter": 1.0, "Kroupa": 0.66, "Chabrier": 0.61}
...
def bin_tab(
    tab, x_name, y_name, x_bins, y_bins, N_thresh=0.0, smooth_sigma=None
):
    x = tab[x_name]
    y = tab[y_name]
    stat, x_bin_edges, y_bin_edges, bin_no = binned_statistic_2d(
        x, y, None, "count", bins=[x_bins, y_bins], expand_binnumbers=True
    )
...
def calc_contamination(
    ...
    y_name,
    z_label,
    crop_key="final_sample_highz_fsps_larson",
    epsilon_lim=0.15,
    z_bin=[6.5, 13.0],
    frame="obs",
    sim="jaguar",
    ...
    tab = tab[
        (
            (tab[z_label] > z_bin_[0])
            & (tab[z_label] < z_bin_[1])
            & (tab[crop_key])
        )
    ]
    if "epsilon" not in tab.columns:
        tab["epsilon"] = (
            (1.0 + tab["zbest_fsps_larson"]) / (1.0 + tab["redshift"])
        ) - 1.0
    if (
        "stellar_mass_50" not in tab.columns
        and (x_name == "mass" or y_name == "mass")
        and frame == "obs"
    ):
        stellar_mass_50_col = [
            mass_int
            + np.random.normal(
                mass_corr_dict[sfh_zprior]["interlopers"]["mu"],
                mass_corr_dict[sfh_zprior]["interlopers"]["sigma"],
            )
            if epsilon > epsilon_lim or epsilon < -epsilon_lim
            else mass_int
            + np.random.normal(
                mass_corr_dict[sfh_zprior]["high-z"]["mu"],
                mass_corr_dict[sfh_zprior]["high-z"]["sigma"],
            )
            for i, (epsilon, mass_int) in enumerate(
                zip(tab["epsilon"], tab[frame_names["int"]["mass"]])
            )
        ]
    ...
    interloper_tab = tab[
        ((tab["epsilon"] > epsilon_lim) | (tab["epsilon"] < -epsilon_lim))
    ]
    ...
    interloper_binned = bin_tab(
        interloper_tab, x_label, y_label, x_bins, y_bins
    )
    ...
    if plot:
        ...
        ax.set_xlabel(var_labels[x_name], fontsize=22.0)
        ax.set_ylabel(var_labels[y_name], fontsize=22.0)
        ...
        if save:
            ...
            fig.suptitle(
                f"{survey}, N={len(interloper_tab)}/{len(tab)}, {zlabel}",
                y=0.95,
                fontsize=22.0,
            )
...
def calc_contamination_beta_binned(
    ax,
    galfind_tab,
    pipes_tab=Table(),
    x_name="M_UV",
    crop_key="EPOCHS_III_certain",
    pipes_IMF="Kroupa",
    cont_IMF="Chabrier",
    plot_survey=None,
    plot=False,
    ...
    if x_name == "mass" and len(pipes_tab) == 0:
        raise (
            Exception(
                "x_name == 'mass' and pipes_tab == 'None'. Could not bin in stellar mass if stellar mass is not given!"
            )
        )
    ...
    name_to_label_dict = {
        "M_UV": "M_UV_1250-3000Angstrom_conv_filt_PL",
        "mass": "stellar_mass_50",
        "beta": "Beta_1250-3000Angstrom_conv_filt_PL",
    }
    ...
    for i, survey in enumerate(np.unique(galfind_tab["FIELDNAME"])):
        galfind_tab_survey = galfind_tab[
            ((galfind_tab["FIELDNAME"] == survey) & (galfind_tab[crop_key]))
        ]
        if len(pipes_tab) > 0:
            pipes_tab_survey = pipes_tab[
                (
                    (pipes_tab["pipes_FIELDNAME"] == survey)
                    & (pipes_tab[crop_key])
                )
            ]
        ...
        if survey != "CLIO" and survey != "SMACS-0723":
            if x_name == "M_UV":
                corr_factor_x = galfind_tab_survey[
                    "auto_corr_factor_UV_EAZY_fsps_larson"
                ]
                # correct this factor
                ...
                corr_factor_x = -2.5 * np.log10(
                    [
                        corr_factor
                        if corr_factor < 10.0
                        else galfind_tab_survey["auto_corr_factor_mass"][i]
                        for i, corr_factor in enumerate(corr_factor_x)
                    ]
                )
                x = np.array(
                    galfind_tab_survey[name_to_label_dict[x_name]]
                    + corr_factor_x
                )
            elif x_name == "mass":
                corr_factor_x = np.log10(
                    galfind_tab_survey["auto_corr_factor_mass"]
                )
                x = np.array(
                    pipes_tab_survey[name_to_label_dict[x_name]]
                    + corr_factor_x
                    + np.log10(MD14_mass_conversion(pipes_IMF, cont_IMF))
                )
            else:
                raise (Exception(""))
            ...
            contam_binned = np.array(
                hf.get(f"{survey_}/beta-{x_name}/contamination_data")
            )
            ...
            print(survey)
            print(
                np.array(x[np.argwhere(x_bin_no >= len(x_bins))]),
                np.array(y[np.argwhere(x_bin_no >= len(x_bins))]),
                np.array(IDs[np.argwhere(x_bin_no >= len(x_bins))]),
            )
            print(
                np.array(x[np.argwhere(y_bin_no >= len(y_bins))]),
                np.array(y[np.argwhere(y_bin_no >= len(y_bins))]),
                np.array(IDs[np.argwhere(y_bin_no >= len(y_bins))]),
            )
            EPOCHS_III_special_cases = {
                "M_UV": {
                    "NGDEEP": {15291: 0.0, 15409: 0.2, 17230: 0.1},
                    "NEP-2": {8980: 0.0},
                    "NEP-3": {16814: 0.25},
                    "NEP-4": {10695: 0.25, 12803: 0.0},
                    "GLASS": {4546: 0.1},
                    "JADES-Deep-GS": {
                        1577: 0.0,
                        4054: 0.3,
                ...
                "mass": {
                    "CEERSP1": {7463: 0.0},
                    "CEERSP2": {7919: 0.0},
                    "CEERSP5": {7520: 0.0, 7857: 0.0},
                    "CEERSP7": {8028: 0.0, 9601: 0.0},
                    "CEERSP8": {2503: 0.0, 2777: 0.0},
                    "JADES-Deep-GS": {29617: 0.0, 36222: 0.0, 36673: 1.0},
                    "NEP-2": {8980: 0.0},
                    "NEP-4": {12803: 0.0},
                    "NGDEEP": {2330: 0.0, 15291: 0.0, 15409: 0.0},
                },
            }
            cont = [
                contam_binned[y_bin - 1][x_bin - 1]
                if x_bin < len(x_bins) and y_bin < len(y_bins)
                else EPOCHS_III_special_cases[x_name][survey][ID]
                for ID, x_bin, y_bin in zip(IDs, x_bin_no, y_bin_no)
            ]
            print(
                IDs[np.where(np.isnan(cont))],
                x[np.where(np.isnan(cont))],
                y[np.where(np.isnan(cont))],
            )
            ...
            if survey_ == plot_survey:
                ...
                scat = ax.scatter(
                    x, y, c=colour, s=300, marker="*"
                )  # , cmap = "viridis", vmin = 0., vmax = 1.)
            # print(cont)
        else:  # set column to -99 for all objects
            cont = np.full(len(galfind_tab_survey), -99.0)
        new_tab = Table(
            {
                "contam_ID": IDs,
                "contam_SURVEY": fieldnames,
                f"cont_beta-{x_name}": cont,
            }
        )
        if i == 0:
            out_tab = new_tab
...
def contam_main():
    surveys = [
        "JADES",
        "El-Gordo",
    ]  # , "CEERS", "El-Gordo", "GLASS", "MACS-0416", "NEP", "NGDEEP"] # ["CLIO", "SMACS-0723"]
    ...
    if plot:
        ...
        pipes_tab = Table.read(
            EPOCHS_III_tab_path, hdu=f"pipes_{pipes_name}".upper()
        )
        for survey, instrument in zip(surveys, instruments):
            fig, ax = plt.subplots(
                ncols=len(x_name_arr), figsize=(12, 6), sharey=True
            )
            for j, (ax_, x_name) in enumerate(zip(ax, x_name_arr)):
                ...
                im = calc_contamination(
                    ...
                    frame=frame,
                    plot=True,
                    save=True if j == len(x_name_arr) - 1 else False,
                )[3]
                if j == 0:
                    cbar = fig.colorbar(
                        im,
                    ...
                    cbar.set_label("Contamination", fontsize=22.0)
    ...
    if save_data:
        ...
        for i, (survey, instrument) in enumerate(zip(surveys, instruments)):
            ...
            for j, x_name in enumerate(x_name_arr):
                contam_binned, x_bins, y_bins = calc_contamination(
                    None,
                    None,
                    survey,
                    instrument,
                ...
                survey_contam_mass_lum = survey_contam.create_group(
                    f"{y_name}-{x_name}"
                )
                survey_contam_mass_lum.create_dataset(
                    "x_bins", data=np.array(x_bins)
                )
                survey_contam_mass_lum.create_dataset(
                    "y_bins", data=np.array(y_bins)
                )
                survey_contam_mass_lum.create_dataset(
                    "contamination_data", data=np.array(contam_binned)
                )
        hf.close()


def add_nan_line(ax, im, x_bins, y_bins, value=0.0, color="red", ls="-."):
    ...
    vlines = np.array(
        list(
            zip(
                np.stack((x_bins[l[:, 0] + 1], y_bins[l[:, 1]])).T,
    ...
    hlines = np.array(
        list(
            zip(
                np.stack((x_bins[l[:, 0]], y_bins[l[:, 1] + 1])).T,
                np.stack((x_bins[l[:, 0] + 1], y_bins[l[:, 1] + 1])).T,
            )
        )
    )
    lines = LineCollection(
        np.vstack((vlines, hlines)), lw=1, colors=color, ls=ls
    )
...
def completeness_main():
    ...
    y_name = "beta"
    N_thresh = 0
    z_range_arr = [[6.5, 8.5], [8.5, 11.0], [11.0, 13.0]]
    ...
    for survey in surveys:
        ...
        for x_name in x_name_arr:
            ...
            fig, ax = plt.subplots(
                nrows=len(z_range_arr),
                figsize=(4, 4 * len(z_range_arr)),
                sharex=True,
            )
            for i, z_range in enumerate(z_range_arr):
                hf_survey_xname_z = hf.create_group(
                    f"{survey}/{x_name}/{z_range}"
                )
                z_label = f"{z_range[0]}<z<{z_range[1]}"
                tab[z_label] = (tab["redshift"] < z_range[1]) & (
                    tab["redshift"] > z_range[0]
                )
                tab[f"{z_label}_selected"] = (
                    (tab[z_label]) & (tab["final_sample_highz_fsps_larson"])
                )
                tab_z_range = tab[tab[z_label]]
                tab_z_range_selected = tab[tab[f"{z_label}_selected"]]

                tot_binned = bin_tab(
                    tab_z_range, x_label, y_label, x_bins, y_bins
                )  # , smooth_sigma = 0.5)
                selected_binned = bin_tab(
                    tab_z_range_selected, x_label, y_label, x_bins, y_bins
                )  # , smooth_sigma = 0.5)
                completeness_binned = selected_binned / tot_binned

                hf_survey_xname_z.create_dataset(
                    "counts_binned", data=tot_binned
                )
                hf_survey_xname_z.create_dataset(
                    "completeness_binned", data=completeness_binned
                )
                ...
                if plot:
                    ...
                    sim_edge = add_nan_line(
                        ax[i], completeness_binned, x_bins, y_bins
                    )
                    ax[i].text(
                        0.95,
                        0.05,
