...
from galfind import Mock_SED_rest, Mock_SED_obs, Data, config

def save_template(sed_obs, out_dir = "/raid/scratch/work/austind/GALFIND_WORK/Bagpipes/template_sets/beta_bias_continuity_bursty_zgauss_no_Lya"):
    try:
...
def collate_beta(out_dir = "/raid/scratch/work/austind/GALFIND_WORK/Bagpipes/template_sets/beta_bias_continuity_bursty_zgauss_no_Lya/spectra_obs"):
    if Path(f"{out_dir}/beta_C94.ecsv").is_file():
        ...
        beta = np.array([Table.read(file).meta["beta_C94"] for file in tqdm(files[:10], total = len(files), desc = "loading beta")])
        output_beta = Table([beta], names = ["beta_C94"], dtype = [float]).write(f"{out_dir}/beta_C94.ecsv", overwrite = True)
    #beta = [val for val in beta if val > -3. and val < -1.5]
    print(beta)
    return beta

def plot_beta(out_dir = "/raid/scratch/work/austind/GALFIND_WORK/Bagpipes/template_sets/beta_bias_continuity_bursty_zgauss_no_Lya/spectra_obs"):
    beta = collate_beta(out_dir)
    beta = [val for val in beta if val < 1. and val > -5.]
...
def main():
    ...
    n_gals = 10_000
    mu, sigma, min_m_UV, max_m_UV = 0., 1., 24., 31.5
    mag_arr = np.random.lognormal(mu, sigma, n_gals * 3)
    m_UV_arr = np.array([mag for mag in max_m_UV - (mag_arr - np.exp(-sigma ** 2)) if mag > min_m_UV and mag < max_m_UV])

    if len(m_UV_arr) < n_gals:
        raise(Exception("Increase number of m_UVs extracted!"))
    for i in tqdm(range(n_gals)):
        try:
            sed_rest = Mock_SED_rest.load_pipes_in_template(m_UV_arr[i], pipes_template_dir, i)
            sed_obs = Mock_SED_obs.from_Mock_SED_rest(sed_rest, sed_rest.meta["redshift"], IGM = None)
