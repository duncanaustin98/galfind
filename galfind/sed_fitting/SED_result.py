...
Stores best-fit SED parameters, uncertainties, and posterior PDFs from SED
fitting
...
class SED_result:
    ...
    properties : `dict`
        Best-fit property values
        (e.g., ``{"z": 1.5 * u.dimensionless_unscaled, ...}``).
    property_errs : `dict`
    ...
    def __repr__(self):
        """Return the official string representation of the SED_result
        object."""
        code_label = (
            self.SED_code
            if isinstance(self.SED_code, str)
            else self.SED_code.label
        )
    ...
    #         f"SED_result.__getattr__ {origin=} not in "
    #         f"['SED_result', 'phot_rest']!"
    #     )
    ...
    #             "pdf",
    #         ], galfind_logger.critical(
    #             f"{property_type=} not in "
    #             f"['val', 'errs', 'l1', 'u1', 'pdf']!"
    #         )
    ...
    #             err_message = (
    #                 f"{property_name} {property_type} not "
    #                 f"available in SED_result object!"
    #             )
    ...
    #     # return cls(SED_fit_params, gal.phot, properties,
    #     #     property_errs, property_PDFs, SED)


class Galaxy_SED_results:
    """Container for SED fitting results from multiple SED codes for a
    single galaxy.

    Organizes multiple `SED_result` objects (one per SED fitting
    code/configuration)
...
class Catalogue_SED_results:
    ...
    def from_cat(
        ...
        if load_SEDs:
            ...
            cat_SED_paths = None
        # these arrays are have length == len(cat), and may in
        # principle included None's where SED fitting has now been
        # performed
    ...
    def from_fits_cat(
        ...
        if (
            ...
            and phot_cat is not None
            and cat_creator is not None
        ):
            ...
            phot_arr is not None
            and instrument is None
            and phot_cat is None
            and cat_creator is None
        ):
            pass
        else:
            galfind_logger.critical(
        ...
        # with each element a dict of galaxy properties for the entire
        # catalogue with values of arrays of len(fits_cat)
        # to array of len(fits_cat), with each element an array of
        # len(SED_fit_params_arr) containing a dict of properties for
        # a single galaxy
        labels_arr = [
            {
        ...
        # convert cat_property_errs from array of len(SED_fit_params_arr),
        # with each element a dict of galaxy properties for the entire
        # catalogue with values of arrays of len(fits_cat)
        # to array of len(fits_cat), with each element an array of
        # len(SED_fit_params_arr) containing a dict of properties for
        ...
        # adjust errors if required (i.e. if 16th and 84th percentiles
        # rather than errors)
        cat_property_errs = [
            {
                gal_property: list(
