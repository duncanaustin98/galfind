...
class LePhare(SED_code):
    """`SED_code` wrapper around the external LePhare photo-z /
    SED-fitting tool.
    ...
    def label(self) -> str:
        ...
        """
        # TODO: Full name should probably be different to this,
        # depending on template set used
        return (
            f"{self.__class__.__name__}_{self.SED_fit_params['GAL_TEMPLATES']}"
        )
    ...
    def make_in(
        ...
        if not Path(in_path).is_file() or overwrite:
            ...
            phot, phot_err = self._load_phot(
                ...
                / (
                    u.s * u.cm**2 * u.Hz
                ),  # u.nJy, #u.erg / (u.s * u.cm ** 2 * u.Hz), #u.Jy,
                # u.ABmag,
                -99.0,
            ...
            # calculate context
            contexts = self._calc_context(
                phot, phot_err
            )  # cat, cat.filterset, aper_diam,
    ...
    def _compile_binary(self: Self, type: str) -> None:
        ...
        if not Path(output_bin_name).is_file():
            if type == "GAL":
                ...
                age_path = ""
            config_name = config["LePhare"]["LEPHARE_CONFIG_FILE"].split("/")[
                -1
            ]
            input = [
            ...
            funcs.change_file_permissions(
                output_bin_name.replace(".bin", ".doc")
            )
            if Path(output_bin_name.replace(".bin", ".phys")).is_file():
                funcs.change_file_permissions(
                    output_bin_name.replace(".bin", ".phys")
                )
        else:
    ...
    def _get_bin_out_path(self: Self, type: str) -> str:
        return (
            f"{os.environ['LEPHAREWORK']}/lib_bin/"
            + self.SED_fit_params[f"{type}_TEMPLATES"]
            + ".bin"
        )
    ...
    def _compile_filters(self: Self) -> None:
        ...
        if not Path(save_path).is_file():
            ...
            galfind_logger.info(
                f"Compiling LePhare filters for {extra_log_out}!"
            )
            process = subprocess.Popen(input)
            process.wait()
            galfind_logger.info(
                "Finished compiling LePhare "
    ...
    def _make_filt_txt(self: Self, filt: Filter) -> None:
        ...
        if Path(save_path).is_file():
            galfind_logger.debug(
                f"LePhare filter for {repr(filt)} already exists"
            )
        else:
            galfind_logger.info(f"Making LePhare filter for {repr(filt)}")
            # np.vstack([np.array(filt.wav.to(u.AA).value),
            # np.array(filt.trans)]).T
            out_filt = np.column_stack(
                (np.array(filt.wav.to(u.AA).value), np.array(filt.trans))
            )
            np.savetxt(
                save_path,
                out_filt,
                header=f"{filt.instrument_name}/{filt.filt_name}",
                comments="# ",
            )
            galfind_logger.info(
                f"Saved LePhare filters for {repr(filt)} to {save_path}"
            )
            funcs.change_file_permissions(save_path)
    ...
    def _get_save_filterset_name(self: Self) -> str:
        if self.SED_fit_params["COMPILE_SURVEY_FILTERS"]:
            return (
                "+".join([filt.filt_name for filt in self.filterset]) + ".filt"
            )
        else:
    ...
    def _compile_templates(
        ...
        if not Path(save_path).is_file() or overwrite:
            ...
            process.wait()
            galfind_logger.info(
                "Finished compiling LePhare templates "
    ...
    def _calc_context(
        ...
        # TODO: Update for the case where some galaxies have no data
        # for a specific band!
        ...
        contexts = [
            int(
                np.sum(
                    [
                        2**i
                        for i, (p, e) in enumerate(zip(phot_, err_))
    ...
    # Currently black box fitting from the lephare config path.
    ...
    def fit(
        ...
        **kwargs : `dict`
            ...
            this fit.
        """
        fits_out_path = self.get_fits_out_path(
            cat, aper_diam, save_name=save_name
        )
        ...
        if not Path(fits_out_path).is_file() or overwrite:
            ...
            lephare_config_path = (
                f"{config['LePhare']['LEPHARE_CONFIG_DIR']}/default.para"
            )
    ...
    def _assert_filterset(
        ...
        if not all(
            [
                filt_name in self.filterset.filt_names
    ...
    def make_fits_from_out(
        ...
        **kwargs : `dict`
        ...
        """
        fits_out_path = self.fits_out_path_from_out_path(
    ...
    def get_in_path(
        ...
        """Construct the path to the LePhare ``.in`` input catalogue
        for a catalogue.
        ...
        in_dir = (
            f"{config['LePhare']['LEPHARE_DIR']}/input/"
            + f"{cat.filterset.instrument_name}/{cat.version}/"
            + f"{cat.survey}"
        )
    ...
    def get_out_path(
        ...
        """Construct the path to the LePhare ``.out`` output
        catalogue for a catalogue.
        ...
        out_name = funcs.split_dir_name(in_path, "name").replace(".in", ".out")
    ...
    def fits_out_path_from_out_path(
        ...
        **kwargs : `dict`
        ...
        return out_path.replace(".out", f"_LePhare_{suffix}.fits")
    ...
    def get_spec_path(
        ...
        """Construct the path to a single galaxy's LePhare ``.spec``
        SED/PDF file.
        ...
        return fits_out_path.replace(
            ".fits", f"_SEDs/Id{'0' * (9 - len(str(ID))) + str(ID)}.spec"
        )
    ...
    def get_PDF_paths(
        ...
        PDF_paths = {
            "z": [self.get_spec_path(fits_out_path, gal.ID) for gal in cat]
        }
        return PDF_paths
    ...
    def get_lib_name(
        ...
        """Get the name of the compiled template library for a given
        source type.
        ...
        with open(SED_path) as f:
            ...
            while True:
                ...
                if not reached_pdf and any(
                    ...
                    SED_len = int(
                        [ele for ele in line.split(" ") if ele != ""][1]
                    )
                    SED_z = float(
                        [ele for ele in line.split(" ") if ele != ""][5]
                    )
                ...
                if line.startswith("  0.00000"):
                    reached_pdf = True
                    wanted_SED = [
                        fmt for fmt in SED_fmt if fmt[0].startswith(type)
                    ][0]
    ...
    def extract_PDFs(
        ...
        """Extract posterior PDFs for a galaxy property from LePhare
        output files.
        ...
        with open(PDF_path) as f:
            ...
            while True:
                ...
                if reached_output_format:
                    ...
                    if (
                        i == PDF_len
                    ):  # line.startswith(f" {zmax:.5f}"): # end at zmax
                        break
            f.close()
...
#         data_err = np.array([np.array(cat[col_name + "68_LOW"]),
#             np.array(cat[col_name + "68_HIGH"])])
