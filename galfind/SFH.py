...
class SFH:
    ...
    def from_pipes_post(cls: Self, path: str) -> Self:
        ...
        hubble_time = pipes.utils.age_at_z[pipes.utils.z_array == 0.0]
        # Set up the age sampling for internal SFH calculations.
        log_age_max = np.log10(hubble_time) + 9.0 + 2 * log_sampling
        ages = 10 ** np.arange(6.0, log_age_max, log_sampling) * u.yr
    ...
    def age_of_universe(self: Self) -> u.Quantity:
        ...
        return (
            1.0e9
            * np.interp(self.z, pipes.utils.z_array, pipes.utils.age_at_z)
            * u.yr
        )
    ...
    def plot(
        ...
        **plot_kwargs
        ...
        ax.plot(x, sfh_post[:, 1], **plot_kwargs)
        ...
        ax.fill_between(
            x, sfh_post[:, 0], sfh_post[:, 2], **fill_between_kwargs
        )
        ...
        if annotate:
            if plot_type == "lookback":
                ...
                if ax.get_xlim()[0] < 0:
                    ax.set_xlim(
                        0, self.age_of_universe.to(time_units).value * 1.1
                    )
                else:
                    ax.set_xlim(
                        0,
                        np.max(
                            [
                                self.age_of_universe.to(time_units).value
                                * 1.1,
                                ax.get_xlim()[1],
                            ]
                        ),
                ...
                ax.set_ylim(
                    1e-2,
                    np.max([ax.get_ylim()[1], 1.1 * np.max(sfh_post[:, 2])]),
                )
            elif plot_type == "absolute":
                ...
                if z_axis:
                    ...
                    Gyr_ticks = (
                        np.interp(
                            zvals, pipes.utils.z_array, pipes.utils.age_at_z
                        )
                        * u.Gyr
                    )
            ...
            ax.set_ylabel(
                r"SFR / $\mathrm{{M}}_{{\odot}} \mathrm{{yr}}^{{-1}}$"
            )
            if label_z:
                ax.text(
                    0.1,
        ...
        if zoom_time is not None:
            ...
            zoom_ax.xaxis.label.set_fontsize(
                ax.xaxis.label.get_fontsize() * 0.5
            )
            zoom_ax.yaxis.label.set_fontsize(
                ax.yaxis.label.get_fontsize() * 0.5
            )
            ...
            #     label.set_fontsize(
            #         label.get_yticklabels()[0].get_fontsize() * 0.5
            #     )
            # halve x and y ticks sizes
            zoom_ax.tick_params(
                axis="x",
                which="major",
                labelsize=zoom_ax.get_xticklabels()[0].get_fontsize() * 0.5,
            )
            zoom_ax.tick_params(
                axis="y",
                which="major",
                labelsize=zoom_ax.get_yticklabels()[0].get_fontsize() * 0.5,
            )

            # Connect inset to main plot
            # Add rectangle on the main plot
            rect = Rectangle(
                (ax.get_xlim()[0] - zoom_ax.get_xlim()[1], 0.0),
                zoom_ax.get_xlim()[1],
                zoom_ax.get_ylim()[1],
                edgecolor="purple",
                linewidth=1,
