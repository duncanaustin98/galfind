...
class Prior(ABC):
    ...
    def __init__(
        self: Self, name: str, prior_params: Dict[str, float], fiducial: float
    ):
        self.name = name
    ...
    @abstractmethod
    def __call__(self: Self, param: float) -> bool:
        pass
...
class Flat_Prior(Prior):
    ...
    def get_initpos(
        ...
        return np.random.uniform(
            self.prior_params["lower_lim"], self.prior_params["upper_lim"], 1
        )

    def __call__(self: Self, param: float) -> float:
        if (
            param > self.prior_params["lower_lim"]
            and param < self.prior_params["upper_lim"]
        ):
            return 0.0
            # -np.log(self.prior_params["upper_lim"]
            # - self.prior_params["lower_lim"])
        else:
            return -np.inf
...
class Gaussian_Prior(Prior):
    ...
    def get_initpos(
        ...
        return np.random.normal(
            self.prior_params["mu"], self.prior_params["sigma"], 1
        )

    def __call__(self: Self, param: float) -> float:
        return (
            -0.5
            * ((param - self.prior_params["mu"]) / self.prior_params["sigma"])
            ** 2
        )
...
class Priors:
    ...
    def __call__(self: Self, params: Dict[str, float]) -> float:
    ...
    def __getitem__(self: Self, name: str) -> Prior:
    ...
    def __iter__(self: Self) -> Priors:
        self._iter_index = 0
        return self

    def __next__(self: Self) -> Prior:
...
class Base_MCMC_Fitter(ABC):
    ...
    def get_autocorr_time(self) -> int:
        ...
        try:
            ...
            galfind_logger.warning(
                "Could not calculate autocorrelation time! "
                f"Chains likely unconverged! {e}"
            )
    ...
    def _instantiate_walkers(self: Self) -> NoReturn:
        ...
        # flat_priors_lower = [self.flat_priors[i][0]
        #     for i in range(self.ndim)]
        # flat_priors_diff = [self.flat_priors[i][1]
        #     - self.flat_priors[i][0] for i in range(self.ndim)]

        # pos = [flat_priors_lower + (flat_priors_diff) \
        #     * np.random.uniform(0, 1, self.ndim)
        #     for i in range(self.nwalkers)]
        if not hasattr(self, "init_pos"):
            # init_pos = [self.fiducial_params
            #     + 1e-4 * np.random.uniform(0, 1, self.ndim) * \
            #         self.fiducial_params for i in range(self.nwalkers)]
            init_pos = [
                np.array([prior.get_initpos()[0] for prior in self.priors])
                for i in range(self.nwalkers)
            ]
            self.init_pos = init_pos
    ...
    def __call__(
        ...
        with mp.Pool(processes=n_processes) as pool:
            ...
            if hasattr(self, "backend"):
                galfind_logger.info(
                    "Final size: {0}".format(self.backend.iteration)
                )
        pool.close()
    ...
    def _fix_params(self: Self, params: Dict[str, float]) -> Dict[str, float]:
    ...
    #     self.scatter = np.sqrt(np.sum(residuals ** 2)
    #         / (len(self.y_data) - 1))
    ...
    #         self.params_med = {prior.name: np.median(chain[:, i])
    ...
    def plot(
        ...
        **kwargs : `dict`
        ...
        l1_chains, med_chains, u1_chains = self._get_plot_chains(
            x_arr=x_arr, log_data=log_data, **kwargs
        )
        ax.plot(x_arr - xoff, med_chains, **def_plot_kwargs)
        ax.fill_between(
            x_arr - xoff, l1_chains, u1_chains, **def_fill_between_kwargs
        )
        galfind_logger.info("Plotting MCMC fit")
        return x_arr, med_chains, l1_chains, u1_chains

    def get_sample(
        self: Self, discard: bool = True, thin: bool = True
    ) -> NDArray[float]:
    ...
    def get_chains(
        ...
        **kwargs: Dict[str, Any],
        ...
        """Evaluate `model` over (
            a subset of) the posterior chain at ``x_arr``.
        ...
        **kwargs : `dict`
        ...
        if log_data:
            ...
            def model(x, params):
                return np.log10(self.model(x, params, **kwargs))
        else:
            model = self.model
        if shape is None:
            params_arr = self.sampler.get_chain(
            ...
            params_dict_arr = [
                {
                    **{
                        prior.name: param
                        if not prior.name == "c"
                        else np.random.normal(param, scatter[0])
                        if param < self.get_params_med()[prior.name]
                        else np.random.normal(param, scatter[1])
        ...
        y_fit = np.array(
            [
                model(x_arr, params_dict, **kwargs)
                for params_dict in tqdm(
                    params_dict_arr,
                    desc="Loading chains",
                    total=len(params_dict_arr),
                )
            ]
        )
        return y_fit
    ...
    def plot_corner(
        ...
        **plot_kwargs : `dict`
        ...
        flat_samples = self.backend.get_chain(
            flat=True, **get_chain_kwargs
        )  # discard = discard, thin = thin)
        ...
        if "sigma_arr" in default_plot_kwargs.keys():
            if "levels" not in default_plot_kwargs.keys():
        ...
        if fid_colour is not None:
            ...
            corner.overplot_points(
                fig_,
                np.array(self.fiducial_params)[None],
                marker="s",
                color=fid_colour,
            )
        # calculate best fit values of the variables and their associated
        # errors
        # means = []
        # l1_errs = []
        # u1_errs = []
        ...
        if legend:
            ...
            handles = [
                ...
                plt.Line2D(
                    [], [], color="red", linestyle="dashed", label="Dataset 2"
                ),
            ]
            fig_.legend(
                handles=handles,
    ...
    #     def get_residuals(self: Self, params: Dict[str, float])
    #         -> NDArray[float]:
    ...
    #     sigma: float = 3.0
    # ) -> NDArray[bool]:
    #     removed_residuals = sigma_clip(
    #         self.get_residuals(self.get_params_med()),
    #         sigma = sigma, masked = True)
    #     kept_residuals = ~removed_residuals.mask
    #     return kept_residuals


class MCMC_Fitter(Base_MCMC_Fitter):
    """`Base_MCMC_Fitter` subclass assuming (
        possibly asymmetric) Gaussian y-errors.
    ...
    #     * self.fiducial_params for i in range(self.nwalkers)]
    # # init_pos =
    # [np.array([np.random.uniform(prior.prior_params["lower_lim"], \
    ...
    # @abstractmethod
    # def update(self):
    #     """Update any derived/cached fitter state
    #     (e.g. after loading a chain).
    #
    #     Abstract method to be implemented by subclasses.
    #     """
    #     pass
    ...
    def _get_sigma_sq(
        ...
        sigma = [
            y_err_l1 if residual > 0.0 else y_err_u1
            for residual, y_err_l1, y_err_u1 in zip(
                residuals, self.y_data_errs[0], self.y_data_errs[1]
            )
        ]
    ...
    def log_likelihood(
        ...
        """Compute the log-posterior (
            prior + Gaussian likelihood) for ``params``.
        ...
        #     return return_value
        # else:
        # # append log likelihood to front of blobs array and change to
        tuple
        #     output_values = tuple(np.insert(
        #         self.blob_funcs(params_loc), 0, return_value))
        # # print(return_value, " ".join(
        #     map(str, self.blob_funcs(params_loc))))
        #     # z = (self.blob_funcs(params_loc)[i]
        #     # for i in range(len(self.blob_keys)))
        #     # x = tuple(self.blob_funcs(params_loc))
        #     # print(x)
        #     # print(type(x))
        #     #print(val + "," for val in self.blob_funcs(params_loc))
        #     return output_values #return_value, " ".join(
        #         map(str, self.blob_funcs(params_loc)))
    ...
    def get_scatter(
        ...
        **kwargs,
        ...
        """Compute the intrinsic scatter of the fit,
        according to `scatter_type`.
        ...
        **kwargs : `dict`
        ...
        if self.scatter_type == "asymmetric":
            ...
            positive_scatter = np.sqrt(
                np.sum(positive_residuals**2) / (len(positive_residuals) - 1)
            )
            negative_scatter = np.sqrt(
                np.sum(negative_residuals**2) / (len(negative_residuals) - 1)
            )
            scatter = [positive_residuals, negative_residuals]
            galfind_logger.info(
                f"Scatter: [+{positive_scatter:.3f}, "
                f"-{negative_scatter:.3f}] dex"
            )
        elif self.scatter_type == "scatter":
            # residuals = self.get_residuals(self.get_params_med(), **kwargs)
            # scatter = np.sqrt(np.sum(residuals ** 2) / (len(self.y_data)
            # - 1))
            ...
            if self.scatter_type == "scatter_y":
                y_above_sigma = np.abs(
                    med_params["scatter_y_a"]
                    + y_sorted_above * med_params["scatter_y_b"]
                )
                y_below_sigma = np.abs(
                    med_params["scatter_y_a"]
                    + y_sorted_below * med_params["scatter_y_b"]
                )
            elif self.scatter_type == "scatter_xy":
                y_above_sigma = np.abs(
                    med_params["scatter_c"]
                    + x_sorted_above * med_params["scatter_x"]
                    + y_sorted_above * med_params["scatter_y"]
                )
                y_below_sigma = np.abs(
                    med_params["scatter_c"]
                    + x_sorted_below * med_params["scatter_x"]
                    + y_sorted_below * med_params["scatter_y"]
                )
            elif self.scatter_type == "exp_scatter_y":
                y_above_sigma = np.log1p(
                    np.exp(
                        med_params["exp_scatter_y_a"]
                        + (
                            (y_sorted_above - self.y_data.mean())
                            / self.y_data.std()
                        )
                        * med_params["exp_scatter_y_b"]
                    )
                )
                y_below_sigma = np.log1p(
                    np.exp(
                        med_params["exp_scatter_y_a"]
                        + (
                            (y_sorted_below - self.y_data.mean())
                            / self.y_data.std()
                        )
                        * med_params["exp_scatter_y_b"]
                    )
            ...
            x_roll_above = (
                x_sorted_above[(window_above // 2) : -(window_above // 2)]
                if window_above % 2 == 1
                else x_sorted_above[
                    (window_above // 2 - 1) : -(window_above // 2)
                ]
            )
            x_roll_below = (
                x_sorted_below[(window_below // 2) : -(window_below // 2)]
                if window_below % 2 == 1
                else x_sorted_below[
                    (window_below // 2 - 1) : -(window_below // 2)
                ]
            )
            ...
            scatter = None
            galfind_logger.warning(
                f"{self.scatter_type=} must be in ['asymmetric', "
                "'symmetric', 'scatter_y', 'scatter_xy']. Skipping!"
            )
    ...
    def get_empirical_scatter(
        ...
        bins = np.linspace(
            self.x_data.min(), self.x_data.max(), n_bins
        )  # int(len(x_arr) / 10_000))
        ...
        for i in tqdm(
            range(1, len(bins)),
            desc="Calculating scatter in bins",
            total=len(bins) - 1,
        ):
            ...
            bin_y_errs = np.array(
                [
                    y_l1_ if y_ >= model_y_ else y_u1_
                    for y_, model_y_, y_l1_, y_u1_ in zip(
                        bin_y, bin_model_y, bin_y_errs_l1, bin_y_errs_u1
                    )
                ]
            )
            ...
            # print(f"Bin {i}: median model = {med_model}, median
            # values = {med_values}, N = {len(bin_y)}")
            plot_bin_centres.append(bin_centres[i - 1])
            sigma_l1_emp = (
                quantiles[1] - quantiles[0]
            )  # med_model - np.percentile(bin_y, 16)
            sigma_u1_emp = (
                quantiles[2] - quantiles[1]
            )  # np.percentile(bin_y, 84) - med_model
            # print(np.mean(bin_y_errs_u1[bin_y >= bin_model_y]),
            # np.mean(bin_y_errs_l1[bin_y < bin_model_y]))
            scat_up_ = sigma_u1_emp
            # ** 2 - np.mean(bin_y_errs_u1[bin_y >= bin_model_y]) ** 2
            scat_lo_ = sigma_l1_emp
            # ** 2 - np.mean(bin_y_errs_l1[bin_y < bin_model_y]) ** 2
    ...
    def get_params_errs(self: Self) -> NDArray[float]:
        ...
        if not hasattr(self, "params_errs"):
            chain = self.get_sample()
            params = {
                prior.name: np.percentile(chain[:, i], [16, 50, 84])
                for i, prior in enumerate(self.priors)
            }
            self.params_errs = {}
            for key, vals in params.items():
                self.params_errs[key] = np.array(
    ...
    def sigma_clip(self: Self, sigma: float = 3.0) -> NDArray[bool]:
        ...
        """
        removed_residuals = sigma_clip(
            self.get_residuals(self.get_params_med()), sigma=sigma, masked=True
        )
        kept_residuals = ~removed_residuals.mask
        return kept_residuals
    ...
    def from_h5(cls: Type[Self], h5_path: str) -> Self:
        ...
        """
        galfind_logger.warning(
            "Loading MCMC fitter from HDF5 is not implemented yet, "
            "if it works its a hack to get working for thesis!"
        )
        # construct priors from saved .h5 file
        h5_file = h5py.File(h5_path, "r")
        h5_priors = h5_file["priors"]
        prior_types = {
            prior.__name__: prior for prior in Prior.__subclasses__()
        }
        priors = Priors(
            [
                prior_types[
                    str(
                        h5_priors[prior_name]["prior_type"][()].decode("utf-8")
                    )
                ](
                    prior_name,
                    [
                ...
                for prior_name in [
                    name.decode("utf-8")
                    for name in h5_file["priors_names"][()]
                ]
            ]
        )
        ...
        # fixed_params =
        json.loads(h5_file["fixed_params"][()])  # .decode("utf-8"))
        h5_file.close()
        return cls(
            priors,
            x_data,
            ...
            fixed_params={},
        )  # , incl_scatter = incl_scatter)#, init_pos = init_pos)
    ...
    def save_h5(self: Self) -> str:
        ...
        out_file.create_dataset(
            "priors_names", data=np.array(priors_names, dtype="S")
        )
        ...
        out_file.create_dataset(
            "fixed_params", data=json.dumps(self.fixed_params)
        )
...
#         y_data_errs = None
#         super().__init__(priors, x_data_chains, y_data_chains,
#             y_data_errs, nwalkers, backend_filename, fixed_params)
...
#         else:
#             model = self.model
#         params_arr = [{prior.name: param for prior, param in
#             zip(self.priors, params)} \
#             for params in self.sampler.get_chain(
#                 flat = True, discard = discard, thin = thin)]
...
#             probs_samples = stats.norm(
#                 model, params_loc["scatter"]).pdf(self.y_data)
...
class Schechter_Lum_Fitter(MCMC_Fitter):
    ...
    def model(
        ...
        **kwargs : `dict`
        ...
        """
        return (
            10 ** params["log10_phi_star"]
            * ((x / params["L_star"]) ** params["alpha"])
            * np.exp(-x / params["L_star"])
            / params["L_star"]
        )

    def update(self: Self) -> NoReturn:
...
class DPL_Mag_Fitter(MCMC_Fitter):
    """`MCMC_Fitter` implementing a double power-law function in
    magnitude space.
    ...
    def model(
        ...
        **kwargs : `dict`
        ...
        denominator = 10 ** (
            -0.4 * params["alpha"] * (params["M_star"] - x)
...
class Linear_Fitter(MCMC_Fitter):
        ...
        if len(input_scatter_names) == 0:
            ...
            if all(
                [
                    ...
                    for name in ["scatter_up", "scatter_lo"]
                ]
            ):
                ...
                [
                    name in input_scatter_names
                    for name in ["scatter_y_a", "scatter_y_b"]
                ]
            ):
                scatter_type = "scatter_y"
                # NOT GENERAL!
            elif all(
                [
                    name in input_scatter_names
                    for name in ["exp_scatter_y_a", "exp_scatter_y_b"]
                ]
            ):
    ...
    def _get_sigma_sq(
        ...
        if self.scatter_type is None:
            ...
            return (
                super()._get_sigma_sq(residuals, params)
                + params["scatter"] ** 2
            )
            ...
            return (
                ...
                + (
                    params["scatter_f"]
                    * (10 ** -(self.y_data * (10 ** params["scatter_logk"])))
                )
                ** 2
            )
            ...
            return (
                super()._get_sigma_sq(residuals, params)
                + (params["scatter_y_a"] + self.y_data * params["scatter_y_b"])
                ** 2
            )
        elif self.scatter_type == "exp_scatter_y":
            # softplus
            return (
                ...
                + (
                    np.log1p(
                        np.exp(
                            params["exp_scatter_y_a"]
                            + (
                                (self.y_data - self.y_data.mean())
                                / self.y_data.std()
                            )
                            * params["exp_scatter_y_b"]
                        )
                    )
                )
                ** 2
    ...
    def plot(
        ...
        **kwargs: Dict[str, Any],
        ...
        """Plot the median linear fit,
        its 1-sigma band, and optionally scatter.
        ...
        **kwargs : `dict`
        ...
        if plot_scatter:
            ...
            if x_arr is None:
                x_arr = np.linspace(
                    np.min(self.x_data), np.max(self.x_data), 100
                )
                l1_chains_scat, med_chains, u1_chains_scat = (
                    self._get_plot_chains(x_arr=x_arr, log_data=log_data)
                )
            if self.scatter_type is None:
                galfind_logger.info(
                    "Plotting empirical scatter not included in MCMC fit"
                )
                ...
                if self.scatter_type == "scatter":
                    ...
                    # deliberately wrong way round, as sign convention
                    # in log likelihood also swapped
                    l1_chains_scat -= med_params["scatter_up"]
                    u1_chains_scat += med_params["scatter_lo"]
                elif self.scatter_type in [
                    "scatter_y",
                    "scatter_xy",
                    "exp_scatter_y",
                ]:
                    y_interp_below, y_interp_above = self.get_scatter(
                        x_arr=x_arr
                    )
                ...
                # ax.plot(x_arr, med_chains, color = colour,
...
class Power_Law_Fitter(MCMC_Fitter):
    ...
    def _get_sigma_sq(
        self: Self, residuals: NDArray[float], params: Dict[str, float]
    ) -> NDArray[float]:
        if self.incl_logf:
            return (
                super()._get_sigma_sq(residuals, params)
                + np.exp(2 * params["logf"])
                * self.model(self.x_data, params) ** 2
            )
        else:
            return super()._get_sigma_sq(residuals, params)
        ...
        **kwargs
        ...
        def __init__(
            ...
            **kwargs,
            ...
            # kwargs["priors"] if prior.name != "b_gal"])
        ...
        def _get_sigma_sq(
            ...
            sigma_dm = np.array(
                [self.sigma_dm_dict[survey] for survey in self.surveys_arr]
            )
