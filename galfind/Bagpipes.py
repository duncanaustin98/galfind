...
class Bagpipes(SED_code):
    ...
    def label_from_SED_fit_params(SED_fit_params, short=False):
        ...
        if not short:
            ...
            if SED_fit_params["fix_z"]:
                ...
                if "z_range" in SED_fit_params.keys():
                    ...
                    galfind_logger.critical(
                        f"Bagpipes {SED_fit_params=} must include either 'z_range' if 'fix_z' == False or not included!"
                    )
                    breakpoint()
            # sort SPS label
            ...
            if SED_fit_params["sps_model"].upper() == "BC03":
                ...
                galfind_logger.critical(
                    f"Bagpipes {SED_fit_params=} must include 'sps_model' with .upper() in ['BC03', 'BPASS']"
                )
                breakpoint()
            return (
                f"Bagpipes_sfh_{SED_fit_params['sfh']}_dust_{SED_fit_params['dust']}_"
        ...
        if not Path(fits_out_path).is_file() or overwrite:
            ...
            for name in tab.colnames:
                if name != self.ID_label:
                    tab.rename_column(
                        name, self.galaxy_property_labels(name, SED_fit_params)
                    )
    ...
    def make_fit_instructions_dict(
        ...
        dust_Av: Union[
            tuple, float, None
        ] = None,  # if not None, force overwrite default for given prior / dust type
        ...
        logU: Union[tuple, float, None] = (-3.0, -1.0),
        logU_prior: str = "uniform",
        fesc: Union[tuple, float, None] = (1e-4, 1.0),
        fesc_prior: str = "log_10",
    ):
        fit_instructions = {}
        fit_instructions["t_bc"] = max_birth_cloud_age.to(
            u.Gyr
        ).value  # Max age of birth clouds: Gyr
        exp = {}
        const = {}
        delayed = {}
        burst = {}
        ...
        dblplaw["tau_prior"] = (
            age_prior  # Impose a prior which is uniform in log_10 of the
        )
        ...
        dblplaw["alpha_prior"] = (
            "log_10"  # Impose a prior which is uniform in log_10 of the
        )
        dblplaw["beta_prior"] = (
            "log_10"  # parameter between the limits which have been set
        )
        # above as in Carnall et al. (2017).
        dblplaw["massformed"] = (5.0, 12.0)
        # dblplaw["metallicity"] = (0., 2.5)
        if metallicity_prior == "log_10":
            dblplaw["metallicity"] = (1e-03, 3.0)
        elif metallicity_prior == "uniform":
        ...
        if all([type(name) != type(None) for name in [logU, fesc]]):
            fit_instructions["nebular"] = {
                "logU": logU,
                "logU_prior": logU_prior,
                "fesc": fesc,
                "fesc_prior": fesc_prior,
            }
        # dust
        if all(type(name) != type(None) for name in [dust_prior, dust_type]):
            ...
            if dust_type.lower() == "salim":
                ...
                dust["delta_prior_mu"] = 0.0  # This is Calzetti (approx)
