...
class Combined_Catalogue_Creator:
    ...
    def __init__(
        ...
        open_cat: Optional[
            Callable[[str, str], Optional[Table]]
        ] = open_galfind_cat,
    ):
        self.survey = survey
    ...
    def __repr__(self: Self) -> str:
        ...
        return (
            f"Combined_Catalogue_Creator("
            f"{self.survey}, {self.version}{crop_str})"
        )

    def __str__(self: Self) -> str:
        """Return a detailed string representation of the
        Combined_Catalogue_Creator."""
        output_str = "Combined Catalogue Creator:\n"
    ...
    def crop_name(self) -> List[str]:
        """`list` of `str`: Name(s) describing the crops applied to the
        catalogue."""
...
class Combined_Catalogue(Catalogue_Base):
    ...
    **kwargs
    ...
    def from_cats(
        ...
        if survey is None:
            survey = "+".join(
                sorted(np.unique([cat.survey for cat in cat_arr]))
            )
        if version is None:
            version = "+".join(
        ...
        filters = np.array(
            [
                ...
                for filt in chain.from_iterable(
                    [[filt for filt in cat.filterset] for cat in cat_arr]
                )
            ]
        )
        unique_filters = filters[
            sorted(
                np.unique(
                    [filt.filt_name for filt in filters], return_index=True
                )[1]
            )
        ]
        ...
        if not Path(cat_path).is_file() or overwrite:
            ...
            unique_hdu_names, unique_hdu_counts = np.unique(
                list(
                    chain.from_iterable(
                        [cat.get_hdu_names() for cat in cat_arr]
                    )
                ),
                return_counts=True,
            )
            unique_hdu_names = np.array(
                [
                    hdu
                    for hdu, count in zip(unique_hdu_names, unique_hdu_counts)
                    if count == len(cat_arr)
                ]
            )
            ...
            for i, hdu in enumerate(unique_hdu_names):
                ...
                for j, cat in enumerate(cat_arr):
                    ...
                    # determine SED_code that the hdu originates from,
                    # if not any cat.ID_label
                    ID_colname = []
                    ...
                    # ID_colname = [subcls.ID_label for subcls in
                    # funcs.all_subclasses(SED_code) if
                    # subcls.__name__.upper() in hdu]
                    if len(ID_colname) == 0:
                        ID_colname = [cat.ID_label if i != 0 else "SURVEY_ID"]
                    if len(ID_colname) > 1:
                        # choose the first one found
                        ID_colname_hdu_pos = [
                            hdu.find(subcls.__name__.upper())
                            for subcls in funcs.all_subclasses(SED_code)
                            if subcls.__name__.upper() in hdu
                        ]
                    ...
                    if j == 0:
                        ...
                        cat_unique_ids = np.sum(
                            object_cat_lengths[:j]
                        ) + np.array(list(tab[ID_colname])).astype(int)
                ...
                full_tab["UNIQUE_ID"] = np.array(unique_ids).astype(
                    np.int32
                )  # np.arange(1, len(full_tab) + 1).astype(np.int32)
            ...
            # save table
            Catalogue_Base._write_cat(
                full_tab_hdus, unique_hdu_names, cat_path
            )
    ...
    #     # Check types to allow adding, Catalogue + Multiple_Catalogue,
    #     # Multiple_Catalogue + Catalogue,
    #     # Multiple_Catalogue + Multiple_Catalogue
    #     pass
    ...
    def crop(self: Self, selector: Type[Selector]) -> Self:
        """Apply a selection criterion to this catalogue and its
        ...
        [
            ...
            if selector not in cat.crops
        ]
        self.gals = list(
            chain.from_iterable([cat.gals for cat in self.cat_arr])
        )
    ...
    def _get_unique_IDs(self):
        ...
        return [
            int(
                tab[
                    np.logical_and(
                        (tab["SURVEY_ID"] == gal.ID),
                        (tab["SURVEY"] == gal.survey),
                    )
                ]["UNIQUE_ID"]
            )
            for gal in self
    ...
    def load_sextractor_ext_src_corrs(self):
        """Load SExtractor extended source corrections for every
        ...
        # (the galaxies in self.gals should point to those galaxies
        # stored in the catalogue object)
        if not all(
    ...
    def calc_Vmax(
        ...
        """Calculate and combine maximum observable volumes (Vmax)
        across constituent catalogues.
        ...
        # TODO: Check that the above has loaded in appropriate info
        # for the galaxies!
        ...
        if not Path(save_path).is_file():
            ...
            data = {
                ...
                "aper_diam": np.array(
                    [aper_diam.to(u.arcsec).value] * len(self)
                ),
        ...
        self._load_Vmax_from_ecsv(
            tab, self.survey, aper_diam, SED_fit_code, full_survey_name
        )
    ...
    def plot_combined_area_depth(
        ...
        for pos, band in enumerate(bands):
            ...
            # Set ylim to 2nd / 98th percentile if depth is smaller
            # than this number
            ax.get_ylim()
