...
def open_PSF_model(band, PSF_loc = config["DEFAULT"]["PSF_DIR"], PSF_name = ["PSF_", "cen_G5V_fov299px_ISIM41"]):
    # load PSF .fits image
...
def calc_aper_corr(PSFdata, x_cen, y_cen, band, aper_diam, extract_code = "sep", plot_PSF = True, \
                   PSF_loc = config["DEFAULT"]["PSF_DIR"], PSF_name = ["PSF_", "cen_G5V_fov299px_ISIM41"], print_output = True, tot_aper_size = None):
...
def plot_flux_curve(PSFdata, pixel_scale, x_cen, y_cen, band, flux_pcs, aper_corrs, PSF_loc = config["DEFAULT"]["PSF_DIR"], PSF_name = ["PSF_", "cen_G5V_fov299px_ISIM41"], \
                    save_loc = f"{config['DEFAULT']['GALFIND_WORK']}/Aperture_corrections", tot_aper_size = None, aper_diams = []):
...
def main(in_bands, extract_code = "sep", save_loc = f"{config['DEFAULT']['GALFIND_WORK']}/Aperture_corrections", PSF_loc = config["DEFAULT"]["PSF_DIR"], \
         PSF_name = ["PSF_", "cen_G5V_fov299px_ISIM41"], plot_PSF = True, aper_diams = json.loads(config.get("SExtractor", "APERTURE_DIAMS")) * u.arcsec):
    ...
    for band in in_bands:
        ...
        plot_flux_curve(PSFdata, pixel_scale, x_cen, y_cen, band, flux_pcs, aper_corr, \
                        save_loc = save_loc, PSF_loc = PSF_loc, PSF_name = name, tot_aper_size = tot_aper_size, aper_diams = aper_diams)
        aper_corr.insert(0, band)
        print_line.append(aper_corr)
        # print(print_line)
    np.savetxt(f"{config['DEFAULT']['GALFIND_DIR']}/Aperture_corrections/NIRCam_aper_corr.txt", print_line, fmt = "%s" + len(aper_diams) * " %.6s")
