...
def calc_Inoue14_LS_LAF_optical_depth(lyman_series, wav_obs_arr, z):
    ...
    for j, wav_j in enumerate(np.array(lyman_series["lambda_j"])):
        ...
        wav_indices_2 = (
            (wav_obs_arr < 5.7 * wav_j) & (~wav_indices_1) & (valid_series_indices)
        )
        wav_indices_3 = (~wav_indices_1) & (~wav_indices_2) & (valid_series_indices)
        tau_arr[j, wav_indices_1] = (
            lyman_series["A_j1_LAF"][j] * (wav_obs_arr[wav_indices_1] / wav_j) ** 1.2
        )
        tau_arr[j, wav_indices_2] = (
            lyman_series["A_j2_LAF"][j] * (wav_obs_arr[wav_indices_2] / wav_j) ** 3.7
        )
        tau_arr[j, wav_indices_3] = (
            lyman_series["A_j3_LAF"][j] * (wav_obs_arr[wav_indices_3] / wav_j) ** 5.5
        )
...
def calc_Inoue14_LS_DLA_optical_depth(lyman_series, wav_obs_arr, z):
    ...
    for j, wav_j in enumerate(np.array(lyman_series["lambda_j"])):
        ...
        tau_arr[j, wav_indices_1] = (
            lyman_series["A_j1_DLA"][j] * (wav_obs_arr[wav_indices_1] / wav_j) ** 2
        )
        tau_arr[j, wav_indices_2] = (
            lyman_series["A_j2_DLA"][j] * (wav_obs_arr[wav_indices_2] / wav_j) ** 3
        )
    return np.sum(tau_arr, axis=0)
    ...
    if z > 0.0:
        if z < 1.2:
            ...
            tau[wav_indices_1] = (
                ...
                - (0.25 * ((wav_obs_arr[wav_indices_1] / wav_lyman_lim) ** 2.1))
            )
            ...
            tau[wav_indices_1] = (
                ...
                + (0.325 * (wav_obs_arr[wav_indices_1] / wav_lyman_lim) ** 1.2)
                - (3.14e-2 * (wav_obs_arr[wav_indices_1] / wav_lyman_lim) ** 2.1)
            )
            tau[wav_indices_2] = (
                (
                    5.22e-4
                ...
                - (2.55e-2 * (wav_obs_arr[wav_indices_2] / wav_lyman_lim) ** 3.7)
            )
            tau[wav_indices_3] = 5.22e-4 * (
                (((1 + z) ** 3.4) * (wav_obs_arr[wav_indices_3] / wav_lyman_lim) ** 2.1)
...
def calc_IGM_transmission(
    lyman_series, wav_rest_arr, z, prescription=config["MockSEDs"]["IGM_PRESCRIPTION"]
):
...
def make_IGM_transmission_grid(
    ...
    for i, z in tqdm(
        enumerate(z_arr), total=len(z_arr), desc=f"Making {prescription} IGM grid"
    ):
...
class IGM:
    ...
    def load_IGM_transmission_grid(self):
        with h5py.File(
            f"{config['MockSEDs']['IGM_DIR']}/{self.prescription}_IGM_grid.h5", "r"
        ) as IGM_grid:
    ...
    def interp_transmission(self, z, wav_rest_arr):  # wav_rest_arr should have units
        ...
        transmission_arr = (
            list(np.zeros(len(wav_rest_arr[wav_rest_arr <= wav_lyman_lim] - 1)))
    ...
    def plot_slice(
        ...
        if annotate:
            ...
            if frame == "rest":
                ax.set_xlabel(
                    r"$\lambda_{\mathrm{rest}}~/~\mathrm{%s}$" % wav_rest_arr.unit
                )
                ax.set_xlim(wav_lyman_lim, wav_lyman_alpha)
            else:
                ax.set_xlabel(
                    r"$\lambda_{\mathrm{obs}}~/~\mathrm{%s}$" % wav_rest_arr.unit
                )
