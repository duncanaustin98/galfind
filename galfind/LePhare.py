...
class LePhare(SED_code):
    ...
    def make_in(self, cat, units = u.ABmag, fix_z = False): # from FITS_organiser.py
        lephare_in_path = f"{self.code_dir}/input/{cat.data.instrument.name}/{cat.data.version}/{cat.data.survey}/{cat.cat_name[:-5]}_{cat.cat_creator.min_flux_pc_err}pc.in"
