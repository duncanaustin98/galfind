...
class Catalogue:
    ...
    @classmethod
    def from_NIRCam_pipeline(cls, survey, version, aper_diams, cat_creator, xy_offset = [0, 0], forced_phot_band = "f444W", excl_bands = [], loc_depth_min_flux_pc_errs = [5, 10]):
        # make 'Data' object
        data = Data.from_NIRCam_pipeline(survey, version, excl_bands = excl_bands)
        return cls.from_data(data, aper_diams, cat_creator, xy_offset, forced_phot_band, loc_depth_min_flux_pc_errs)
    
    @classmethod
    def from_data(cls, data, aper_diams, cat_creator, xy_offset = [0, 0], forced_phot_band = "f444W", loc_depth_min_flux_pc_errs = [5, 10]):
        ...
        if cat_creator.cat_type == "loc_depth":
            data.make_loc_depth_cat(aper_diams, min_flux_pc_err_arr = loc_depth_min_flux_pc_errs, forced_phot_band = forced_phot_band)
