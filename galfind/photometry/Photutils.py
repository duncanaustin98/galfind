...
def get_code() -> str:
    ...
    -------
    `str`
        The installed photutils version, formatted as
        ``"photutils-v<version>"``.
    """
...
def get_segmentation_path(
    ...
    """Construct photutils segmentation map path.

    Ensures the directory exists for the segmentation map.
    ...
    -------
    `str`
        The path to the ``.fits`` segmentation map for this
        band/segmentation type.
    """
    photutils_dir = config["Photutils"]["PHOTUTILS_DIR"]
    seg_dir = (
        f"{photutils_dir}/{self.instr_name}/{self.version}/"
        f"{self.survey}/{segment_type}/segmentation"
    )
    seg_path = (
        f"{seg_dir}/{self.survey}_{self.filt_name}_"
        f"{self.filt_name}_{self.version}_seg.fits"
    )
...
def get_forced_phot_path(
    ...
    """Construct forced photometry catalogue path.

    Ensures the directory exists for the forced photometry path.
    ...
    forced_phot_dir = (
        f"{config['Photutils']['PHOTUTILS_DIR']}/{self.instr_name}/"
        + f"{self.version}/{self.survey}/{segment_type}/forced_phot/"
        + funcs.aper_diams_to_str(self.aper_diams)
    )
    forced_phot_path = (
        f"{forced_phot_dir}/{self.survey}_{self.filt_name}"
...
def segment(
    ...
    if (
        not (Path(seg_path).is_file() and Path(source_cat_path).is_file())
        or overwrite
    ):
        galfind_logger.info(
            f"Segmenting {repr(self)} with photutils using {segment_type=}!"
        )
        ...
        if segment_type == "bkg":
            ...
            bkg = Background2D(
                data, box_size, bkg_estimator=MedianBackground(), **bkg_kwargs_
            )
            ...
            bkg_hdu = fits.ImageHDU(
                bkg.background, header=hdr, name="BACKGROUND"
            )
            bkg_rms_hdu = fits.ImageHDU(
                bkg.background_rms, header=hdr, name="BACKGROUND_RMS"
            )
            ...
            galfind_logger.info(
                f"Saved {repr(self)} background map to {bkg_path}"
            )
            threshold = bkg.background_rms * thresh_mult
            thresh_kwargs = bkg_kwargs
        else:
            threshold = detect_threshold(
                data, error=self.load_rms_err()[0], **detect_thresh_kwargs
            )
        ...
        galfind_logger.info(
            f"Saved {repr(self)} segmentation map to {seg_path}"
        )

        # write source catalogue
        wcs = HSTWCS(hdul, self.im_ext)
        input_cat = SourceCatalog(data, segment_map, wcs=wcs)
        ...
        galfind_logger.info(
            f"Saved {repr(self)} source catalogue to {source_cat_path}. "
            f"Found {len(input_cat)} sources."
        )
...
def perform_forced_phot(
    ...
    if not Path(forced_phot_path).is_file() or overwrite:
        ...
        for aper_diam in self.aper_diams.to(u.arcsec).value:
            apertures_ = SkyCircularAperture(
                sky_coords, r=aper_diam * u.arcsec / 2.0
            )
        ...
        for pos, col in enumerate(colnames):
            ...
            phot_table[name] = -2.5 * np.log10(phot_table[col]) + self.ZP
