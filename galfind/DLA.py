...
class DLA:
    ...
    def __init__(self, N_HI, Doppler_b, vel_offset = 0. * u.km / u.s, z_offset = 0., voigt_method = "Tepper-Garcia+06"):
        self.N_HI = N_HI
    ...
    def photon_absorption_const(self):
        return (line_diagnostics["Lya"]["oscillator_strength"] * 4 * np.sqrt(np.pi ** 3) * (const.e.esu ** 2) \
                                / (const.m_e * const.c * line_diagnostics["Lya"]["rel_lambda"])).to(u.cm ** 2)
    
    @property
    def a(self): # DLA damping parameter
        return ((line_diagnostics["Lya"]["line_wav"] ** 2) * line_diagnostics["Lya"]["rel_lambda"] \
                    / (4 * np.pi * const.c * self.delta_lambda)).to(u.dimensionless_unscaled)
    ...
    def tau(self, wav_rest):
        if self.voigt_method == "Tepper-Garcia+06":
            x = (((self.z_vel_offset + self.vel_offset).to(u.AA, equivalencies = u.doppler_relativistic(wav_rest)) \
                  - line_diagnostics["Lya"]["line_wav"]) / self.delta_lambda).to(u.dimensionless_unscaled)
    ...
    def plot_voigt_profile(self, ax, wav_rest):
        ax.plot(wav_rest, Tepper_Garcia06_lyman_alpha_voigt_profile(wav_rest, self.delta_lambda))
