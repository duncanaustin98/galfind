...
def test_expected_instr():
    assert all(
        key in expected_instr_facilities.keys()
        for key in expected_instr_bands.keys()
        if key in instr_names
    )
    assert all(
        key in expected_instr_bands.keys()
        for key in expected_instr_facilities.keys()
        if key in instr_names
    )
    assert all(
        len(values) > 0
        for (key, values) in expected_instr_bands.items()
        if key in instr_names
    )


all_filters = [
    (expected_instr_facilities[instr_name], instr_name, band)
    for instr_name in instr_names
    for band in expected_instr_bands[instr_name]
]
# choose which filters to skip
filters_to_test = [
    pytest.param(_all_filters, marks=pytest.mark.skip)
    if _all_filters[2] not in ["F814W", "F090W"]
    else _all_filters
    for _all_filters in all_filters
]


@pytest.fixture(
    scope="module",
    params=filters_to_test,
    ids=lambda filt_tuple: f"{filt_tuple[0]}/{filt_tuple[1]}.{filt_tuple[2]}",
)
def filter(request):
    return Filter.from_SVO(
...
@pytest.fixture(
    scope="module",
    params=[[], ["F090W"], expected_instr_bands["NIRCam"]],
    ids=NIRCam_excl_bands_ids,
)
...
def test_expected_SVO_instr(instrument):
    ...
    assert all(
        instr_band == expected_band
        for instr_band, expected_band in zip(
            instrument.band_names, expected_instr_bands[instrument.name]
        )
    )
...
def test_instr_filters_match(instrument):
    assert all(
        Filter.from_SVO(instrument.facility, instrument.name, band.band_name)
        == band
...
combined_instr_names = [
    list(chain)
    for chain in itertools.chain.from_iterable(
        itertools.permutations(instr_names, i + 1)
    ...
    if filter.band_name in instrument.band_names:
        ...
        new_band_names = [
            ...
            for band in sorted(
                np.concatenate(
                    [
                        np.array([band for band in instrument]),
                        np.array([filter]),
                    ]
                ),
        ...
        assert all(
            band_name == expected_band_name
            for band_name, expected_band_name in zip(
                new_instrument.band_names, new_band_names
            )
        )
...
def test_instr_instr_add(combined_instrument, NIRCam_instrument):
    ...
    bands_to_add = np.array(
        [
            band
            for band in NIRCam_instrument
    ...
    if len(bands_to_add) == 0:
        ...
        if len(bands_to_add) > 0 and len(bands_to_add) < len(
            NIRCam_instrument
        ):
            # expect warning
            with pytest.warns(UserWarning):
        ...
        assert len(new_instrument) == len(bands_to_add) + len(
            combined_instrument
        )
        assert len(new_instrument.band_names) == len(bands_to_add) + len(
            combined_instrument.band_names
        )
        # ensure the correct filters are present, sorted blue -> red
        new_band_names = [
            band.band_name
            for band in sorted(
                np.concatenate(
                    [
                        np.array([band for band in combined_instrument]),
                        bands_to_add,
                    ]
                ),
                key=lambda band: band.WavelengthCen.to(u.AA).value,
