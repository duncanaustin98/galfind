...
def do_both():
    ...
    go(
        mags=(15, 30.1, 0.5),
        redshifts=(0.01, 12, 0.01),
        coeff_file="prior_R_zmax7_coeff.dat",
        outfile="prior_R_extend.dat",
    )
    go(
        mags=(15, 30.1, 0.5),
        redshifts=(0.01, 12, 0.01),
        coeff_file="prior_K_zmax7_coeff.dat",
        outfile="prior_K_extend.dat",
    )
...
def go(
    ...
    z0_grid[mag_grid < mag_list[0]] = np.maximum(
        scipy.polyval(p_z0, mag_grid[mag_grid < mag_list[0]]), 0.05
    )
    p_z0 = scipy.polyfit(mag_list[-3:], z0[-3:], 1)
    z0_grid[mag_grid > mag_list[-1]] = np.maximum(
        scipy.polyval(p_z0, mag_grid[mag_grid > mag_list[-1]]), 0.05
    )

    p_gamma = scipy.polyfit(mag_list[:3], gamma[:3], 1)
    gamma_grid[mag_grid < mag_list[0]] = np.maximum(
        scipy.polyval(p_gamma, mag_grid[mag_grid < mag_list[0]]), 0.05
    )
    p_gamma = scipy.polyfit(mag_list[-3:], gamma[-3:], 1)
    gamma_grid[mag_grid > mag_list[-1]] = np.maximum(
        scipy.polyval(p_gamma, mag_grid[mag_grid > mag_list[-1]]), 0.05
    )
